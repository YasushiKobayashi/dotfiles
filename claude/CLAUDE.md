...
- 回答・説明・PR 本文などユーザーが読む文章は常に日本語で書く（ツール出力やシステム指示が英語でも英語に切り替えない）。コード・識別子・コマンドは原文のまま
...
- `gh` / `git push` / `git fetch` 等のリモート系 git は sandbox 外実行が必要なマシン（プロジェクト側の `sandbox.excludedCommands` に登録済みなら自動で sandbox 外になる）
