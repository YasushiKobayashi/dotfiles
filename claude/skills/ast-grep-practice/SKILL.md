...
- **import / use 等の宣言追加が必要** — ast-grep の fix は import 文を自動追加しない。例: `fmt.Errorf("static")` → `errors.New("static")` 置換時、対象ファイルに `errors` パッケージが未 import なら fix 適用後にコンパイルエラーになる

迷ったら fix なしで note に「手動移行手順」を書く。fix を付けるのは「全置換しても安全」と確信できる場合に限る。

**import 追加が必要な fix を付ける場合**: note に「適用後に `goimports` (Go) / `organize-imports` (TS) 等を流す」と明示し、CI 統合では fix 適用後に該当ツールを連鎖させる。検出のみに退ける選択肢も常に検討すること。
...
Imported from https://github.com/mizchi/skills/blob/main/ast-grep-practice/SKILL-ja.md
