...
4. **重複チェック（必須）**: 提案前に既存の知見と照合する。重複や近接する規則があれば「新規追加」ではなく「既存への追記 / 更新」を選ぶ。これを怠ると skill / ルールが肥大化する。
   ...
   ```
   # skill 重複（global）
   # ※ ~/.claude/skills/ が不在 or symlink で実体が project 側に集約されている場合は、project の skills/ を global 相当として扱う
   ls ~/.claude/skills/ 2>/dev/null || echo "(global skills 不在 → project skills を実体扱い)"
   Grep "<キー>" ~/.claude/skills/*/SKILL.md
   Grep "<キー>" <project-root>/skills/*/SKILL.md  # symlink/集約パターン用
...
**学びが二面的な場合**: 「短い禁止事項（CLAUDE.md 向き）」と「コマンド例の手順（skill 向き）」の両面を持つ場合、両方を採用候補に並べてユーザーに採否を委ねる。skill 提示フォーマットは「学びは複数あって良い」原則なので分割提案は許容される。
...
プロジェクトに `doc/coding_rules/` のようなコーディングルール SSOT がある場合、新規追加前に既存節があるか `Grep` で確認すること。frontmatter で globs を使って各サービスの CLAUDE.md に自動集約する運用なら、適切な `globs:` を付与し集約コマンドを忘れず実行する。
...
- `empirical-prompt-tuning` — プロンプト開発中のチューニング。本 skill はタスク終了後の学び固定化、empirical はプロンプト開発中、と使い分ける
...
Imported from https://github.com/mizchi/skills/blob/main/retrospective-codify/SKILL-ja.md
英語版は `SKILL-en.md` を参照。
